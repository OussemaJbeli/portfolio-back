<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Api\Concerns\SingletonController;
use App\Models\JourneySection;

class JourneySectionController extends SingletonController
{
    protected string $model = JourneySection::class;

    protected function updateRules(): array
    {
        return [
            'section_badge_en' => ['sometimes', 'string', 'max:80'],
            'section_badge_fr' => ['sometimes', 'string', 'max:80'],
            'section_badge_ar' => ['sometimes', 'string', 'max:80'],
            'heading_en' => ['sometimes', 'string', 'max:160'],
            'heading_fr' => ['sometimes', 'string', 'max:160'],
            'heading_ar' => ['sometimes', 'string', 'max:160'],
            'subheading_en' => ['nullable', 'string', 'max:500'],
            'subheading_fr' => ['nullable', 'string', 'max:500'],
            'subheading_ar' => ['nullable', 'string', 'max:500'],
            'next_label_en' => ['sometimes', 'string', 'max:160'],
            'next_label_fr' => ['sometimes', 'string', 'max:160'],
            'next_label_ar' => ['sometimes', 'string', 'max:160'],
        ];
    }
}
